...
$oceanwp_columns = apply_filters('ocean_related_blog_posts_columns', absint(get_theme_mod('oceanwpdynamictypes_related_columns_' . $typePost, '3')));

// Term
$term_tax = get_theme_mod('oceanwpdynamictypes_blog_related_taxonomy_' . $typePost, '');
...
// Query args
$args = array(
	'posts_per_page' => apply_filters('ocean_related_blog_posts_count', absint(get_theme_mod('oceanwpdynamictypes_related_count_' . $typePost, '3'))),
	'orderby'        => 'rand',
	'post_type'          => $typePost,
...
if ( $oceanwp_related_query->have_posts() ) :
	...
	<div id="related-posts" class="<?php echo esc_attr($classes); ?>">
		...
		<div class="oceanwp-row clr">
			...
			<?php foreach ( $oceanwp_related_query->posts as $post ) :
				setup_postdata($post); ?>
					...
					elseif ( has_post_thumbnail() ) : ?>
						...
						<figure class="related-post-media clr">
							...
							<a href="<?php the_permalink(); ?>" class="related-thumb">
								...
								<?php
								// Image width
								$img_width  = apply_filters('ocean_related_blog_posts_img_width', absint(get_theme_mod('oceanwpdynamictypes_related_img_width_' . $typePost)));
								$img_height = apply_filters('ocean_related_blog_posts_img_height', absint(get_theme_mod('oceanwpdynamictypes_related_img_height_' . $typePost)));
				...
				<?php if ( $oceanwp_columns == $oceanwp_count ) {
									$oceanwp_count = 0;
				} ?>
