...
class Elements {
		...
		if ( ! is_admin() ) {
			if ( ! Helper::is_edit_mode() ) {
				...
				if ( empty($excluded_globals['DCE_Frontend_Navigator']) ) {
					...
					if ( self::$user_can_elementor || ( isset($_GET['dce-nav']) && empty($excluded_globals['DCE_Frontend_Navigator_Visitor']) ) ) {
						self::$user_can_copy = ! empty($excluded_globals['DCE_Frontend_Navigator_Copy']);
	...
	public function add_elementor_navigator( $admin_bar ) {
		...
		$admin_bar->add_menu(array(
			...
			'href' => $href,
			'meta' => array(
				'title' => __('Frontend Navigator'),
				'class' => 'menupop',
			),
		));
		Assets::dce_icon();
		wp_enqueue_style('dce-admin-bar', plugins_url('/assets/css/dce-admin-bar.css', DCE__FILE__), [], DCE_VERSION);
		add_action('wp_footer', [ $this, 'print_frontend_navigator_submenu' ]);
		wp_enqueue_script('dce-admin-bar', plugins_url('/assets/js/dce-admin-bar.js', DCE__FILE__), [], DCE_VERSION);
	}
	...
	public function print_frontend_navigator() {
		...
		wp_enqueue_style('dce-style-editor-navigator', plugins_url('/assets/css/dce-editor-navigator.css', DCE__FILE__), [], DCE_VERSION);
		wp_enqueue_script('dce-script-editor-navigator', plugins_url('/assets/js/dce-editor-navigator.js', DCE__FILE__), [], DCE_VERSION);
		...
		if ( ! empty($elementor_data) ) {
			if ( is_array($elementor_data) ) {
				...
				foreach ( $elementor_data as $ekey => $evalue ) {
										...
										if ( $type == 'template' ) {
											...
											if ( $document ) {
												...
												<?php if ( self::$user_can_elementor ) { ?>
													...
													<dt><?php _e('Author', 'elementor'); ?></dt> <dd><a href="<?php echo \DynamicContentForElementor\Helper::get_user_link($document->get_post()->post_author); ?>" target="_blank"><?php echo get_the_author_meta('display_name', $document->get_post()->post_author); ?></a></dd>
													...
													<?php } ?>
														...
														if ( $modified_author_id && $document->get_post()->post_author != $modified_author_id ) {
															?>
														<dt><?php _e('Modified by', 'elementor'); ?></dt>  <dd><a href="<?php echo \DynamicContentForElementor\Helper::get_user_link($modified_author_id); ?>" target="_blank"><?php echo get_the_author_meta('display_name', $modified_author_id); ?></a></dd>
													<?php }
												} ?>
					...
					if ( self::$user_can_elementor || self::$user_can_copy ) {
						if ( ! empty(self::$elements_settings[ $id ]) ) {
							$element_settings = self::$elements_settings[ $id ];
							if ( $type != 'template' ) {
								$element_settings = Helper::get_element_by_id($id, $template_id);
							}
						}
					}
