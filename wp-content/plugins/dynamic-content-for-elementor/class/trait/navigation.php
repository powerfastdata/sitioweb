			...
			if ( $settings['pagination_show_firstlast'] ) {
				if ( $paged > 2 && $paged > $range + 1 && $showitems < $pages ) {
					if ( ! $search_filter_query ) {
						...
						echo '<a href="' . self::get_wp_link_page_sf(1) . '" class="pagefirst"><i class="' . $icon_firstlast . 'left"></i> ' . __($settings['pagination_first_label'], 'dynamic-content-for-elementor' . '_texts') . '</a>';
					}
				}
			}
			...
			if ( $settings['pagination_show_prevnext'] ) {
				if ( $paged > 1 && $showitems < $pages ) {
					if ( ! $search_filter_query ) {
						...
						echo '<a href="' . self::get_wp_link_page_sf($paged - 1) . '" class="pageprev"><i class="' . $icon_prevnext . 'left"></i> ' . __($settings['pagination_prev_label'], 'dynamic-content-for-elementor' . '_texts') . '</a>';
					}
				}
			}
			...
			if ( $settings['pagination_show_prevnext'] ) {
				if ( $paged < $pages && $showitems < $pages ) {
					if ( ! $search_filter_query ) {
						...
						echo '<a href="' . self::get_wp_link_page_sf($paged + 1) . '" class="pagenext">' . __($settings['pagination_next_label'], 'dynamic-content-for-elementor' . '_texts') . ' <i class="' . $icon_prevnext . 'right"></i></a>';
					}
				}
			}
			//Last
			if ( $settings['pagination_show_firstlast'] ) {
				if ( $paged < $pages - 1 && $paged + $range - 1 < $pages && $showitems < $pages ) {
					if ( ! $search_filter_query ) {
						...
						echo '<a href="' . self::get_wp_link_page_sf($pages) . '" class="pagelast">' . __($settings['pagination_last_label'], 'dynamic-content-for-elementor' . '_texts') . ' <i class="' . $icon_firstlast . 'right"></i></a>';
					}
				}
			}
