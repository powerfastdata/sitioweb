...
class Log {
	...
	public static function http_prepare_download_log() {
		if ( isset( $_GET['wpmdb-download-log'] ) && wp_verify_nonce( sanitize_text_field( $_GET['nonce'] ), 'wpmdb-download-log' ) ) {
			ob_start();
	...
	public static function get_diagnostic_info() {
		...
		$diagnostic_info['server-info'] = array(
			'Web Server'                      => ! empty( $_SERVER['SERVER_SOFTWARE'] ) ? sanitize_text_field( $_SERVER['SERVER_SOFTWARE'] ) : '',
