...
class Settings {
	...
	public function init() {
		...
		if ( is_admin() ) {
			...
			$dce_apis = get_option(DCE_PRODUCT_ID . '_apis');
			if ( ! empty($dce_apis['dce_api_gmaps']) && ! empty($dce_apis['dce_api_gmaps_acf']) ) {
				if ( Helper::is_acfpro_active() ) {
					add_action('acf/init', function() {
						$dce_apis = get_option(DCE_PRODUCT_ID . '_apis');
						acf_update_setting('google_api_key', $dce_apis['dce_api_gmaps']);
					});
				} elseif ( Helper::is_acf_active() ) {
					add_filter('acf/fields/google_map/api', function( $api ) {
						$dce_apis = get_option(DCE_PRODUCT_ID . '_apis');
						$api['key'] = $dce_apis['dce_api_gmaps'];
						return $api;
					});
				}
			}
	...
	public function dce_setting_page( $tplsys = false ) {
		...
		<div class="wrap">
			...
			<div id="dce-settings-tabs-wrapper" class="nav-tab-wrapper">
				<a id="dce-settings-tab-settings" class="nav-tab<?php if ( ! isset($_GET['tab']) || ( isset($_GET['tab']) && $_GET['tab'] == 'settings' ) ) {
				...
				<a id="dce-settings-tab-widgets" class="nav-tab<?php if ( isset($_GET['tab']) && $_GET['tab'] == 'widgets' ) {
					...
					<span class="dce-badge"><?php $widgets = Widgets::get_active_widgets();
				...
				<a id="dce-settings-tab-extensions" class="nav-tab<?php if ( isset($_GET['tab']) && $_GET['tab'] == 'extensions' ) {
					...
					<span class="dce-badge"><?php $extensions = Extensions::get_active_extensions();
			...
			if ( isset($_GET['tab']) && $_GET['tab'] == 'other' ) {
			...
			if ( isset($_GET['tab']) && $_GET['tab'] == 'globals' ) {
		...
		<form action="" method="post">
			...
			if ( isset($_POST['save-dce-widgets']) ) {
				if ( isset($_POST['dce-widgets'] ) ) {
					update_option(DCE_PRODUCT_ID . '_active_widgets', json_encode( sanitize_text_field( $_POST['dce-widgets'] ) ));
				} else {
			...
			foreach ( Widgets::get_active_widgets_by_group() as $key => $value ) {
				if ( $i ) {
					?><?php } ?>
	...
	public function show_extensions_form() {
		...
		<form action="" method="post">
			<?php
			$extensions = Extensions::get_active_extensions();
			if ( isset($_POST['save-dce-extensions'] ) ) {
				if ( isset($_POST['dce-extensions'] ) ) {
					update_option ( DCE_PRODUCT_ID . '_active_extensions', json_encode( sanitize_text_field( $_POST['dce-extensions'] ) ) );
				} else {
	...
	public function show_documents_form() {
		...
		<form action="" method="post">
			<?php
			$documents = Documents::get_active_documents();
			if ( isset($_POST['save-dce-documents'] ) ) {
				if ( isset($_POST['dce-documents'] ) ) {
					update_option(DCE_PRODUCT_ID . '_active_documents', json_encode( sanitize_text_field( $_POST['dce-documents'] )));
				} else {
		...
		<form action="" method="post">
			...
			if ( isset($_POST['save-dce-globals']) ) {
				if ( isset( $_POST['dce-globals'] ) ) {
					update_option ( DCE_PRODUCT_ID . '_active_globals', json_encode( sanitize_text_field( $_POST['dce-globals'] ) ) );
				} else {
				...
				$globals_tmp['dce_frontend_navigator_visitor'] = 'DCE_Frontend_Navigator_Visitor';
				foreach ( $globals_tmp as $key => $className ) {
					if ( ! isset($_POST['dce-globals'][ $className ]) ) {
						$excluded_globals[ $className ] = $key;
					}
				}
				if ( isset($_POST['dce-globals']['DCE_Frontend_Navigator_Copy']) ) {
					$excluded_globals['DCE_Frontend_Navigator_Copy'] = 'dce_frontend_navigator_copy';
				}
			...
			<table class="widefat dce-form-table dce-filter-table">
				...
				<tbody>
					...
					<?php
					$key = 'dce_frontend_navigator_visitor';
					$className = 'DCE_Frontend_Navigator_Visitor';
					?>
					<tr>
						<td> &nbsp; &nbsp; &nbsp; &nbsp;
							<input class="dce-globals dce-globals-<?php echo strtolower($key); ?>" type="checkbox" name="dce-globals[<?php echo $className; ?>]" id="dce-globals-<?php echo $className; ?>"
					...
					<?php
					$key = 'dce_frontend_navigator_copy';
					$className = 'DCE_Frontend_Navigator_Copy';
					?>
					<tr>
						<td> &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp;
							<input class="dce-globals dce-globals-<?php echo strtolower($key); ?>" type="checkbox" name="dce-globals[<?php echo $className; ?>]" id="dce-globals-<?php echo $className; ?>"
							...
							?>>
							<label for="dce-globals-<?php echo $className; ?>">
								<?php _e('Enable Copy Button', 'dynamic-content-for-elementor'); ?>
							</label>
						</td>
					</tr>
				</tbody>
	...
	public function show_apis_form() {
		...
		<form action="" method="post">
			...
			if ( isset($_POST['save-dce-apis']) ) {
				update_option( DCE_PRODUCT_ID . '_apis', sanitize_text_field( $_POST['dce-apis'] ));
				...
				<tbody>
					<tr>
						<td>
							...
							<input class="dce-apis" type="text" name="dce-apis[dce_api_gmaps]" id="dce-apis-gmaps" value="<?php echo ( isset($dce_apis['dce_api_gmaps']) ) ? $dce_apis['dce_api_gmaps'] : ''; ?>"><br>
							<?php if ( Helper::is_acf_active() ) { ?>
							<input class="dce-apis dce-apis-gmaps_acf" type="checkbox" name="dce-apis[dce_api_gmaps_acf]" id="dce-apis-gmaps_acf"<?php echo ( ! empty($dce_apis['dce_api_gmaps_acf']) ) ? ' checked' : ''; ?>> <label for="dce-apis-gmaps_acf"><?php _e('Set this API also in ACF Configuration.', 'dynamic-content-for-elementor'); ?> <a href="https://www.advancedcustomfields.com/blog/google-maps-api-settings/" target="_blank"><?php _e('Why?', 'dynamic-content-for-elementor'); ?></a></label>
							<?php } ?>
	...
	public function show_dts_form() {
		if ( isset($_POST['action']) && $_POST['action'] == 'update' ) {
			update_option(DCE_OPTIONS, sanitize_text_field( $_POST[ DCE_OPTIONS ] ));
			$this->options = sanitize_text_field( $_POST[ DCE_OPTIONS ] );
			Notice::dce_admin_notice__success(__('Your preferences have been saved.', 'dynamic-content-for-elementor'));
		}
		?>
	...
	public function show_dce_dts_form() {
		...
		if ( isset($_POST['action']) && $_POST['action'] == 'update' ) {
			update_option(DCE_OPTIONS, sanitize_text_field( $_POST[ DCE_OPTIONS ] ));
			update_option('dce_template_disable', sanitize_text_field( $_POST['dce_template_disable'] ));
			$this->options = sanitize_text_field( $_POST[ DCE_OPTIONS ] );
			Notice::dce_admin_notice__success(__('Your preferences have been saved.', 'dynamic-content-for-elementor'));
		}
