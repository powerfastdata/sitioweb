...
class DCE_Widget_Pdf extends DCE_Widget_Prototype {
	...
	protected $depended_scripts = [ 'dce-pdf-widget-button' ];
	...
	protected static function js_template_error_alert() { ?>
<script>
function downloadPDF() {
			alert("Error: could not fetch the template for generating the pdf");
			return false;
}
</script>
		<?php
	...
	public function get_settings_for_display($settings_key = NULL) {
...
function downloadPDF(){
	...
	/**
	 * Enqueue the js converter scripts that will immediately
	 * render the pdf and trigger the download.
	 */
	public function jsconv_enqueue_scripts() {
		$downloadPDF = isset($_GET['downloadPDF']) ? sanitize_text_field( $_GET['downloadPDF'] ) : 0;
		if ( $downloadPDF ) {
			wp_enqueue_script('dce-pdf-jsconv');
		}
	}

	private function jsconv_get_settings_json( $psettings ) {
		if ( $psettings['source_type'] === 'template' ) {
			$psettings['selector'] = 'body';
		}
		$loc = [
	...
	protected function render() {
		...
		if ( $settings['dce_pdf_button_converter'] == 'js' ) {
			$psettings = self::polish_settings( $settings );
			$json_settings = $this->jsconv_get_settings_json( $psettings );
			if ( $psettings['source_type'] === 'template' ) {
				$template_link = get_permalink($psettings['template_id']);
				$ch = strpos( $template_link, '?' ) ? '&' : '?';
				$template_link .= $ch . http_build_query( [ 'downloadPDF' => $json_settings ]);
				$this->add_render_attribute( 'button', 'href', $template_link );
			} else {
				$current_link = get_permalink();
				$ch = strpos( $current_link, '?' ) ? '&' : '?';
				parse_str($_SERVER['QUERY_STRING'], $parameters);
				$current_link .= $ch . http_build_query( array_merge( $parameters, [ 'downloadPDF' => $json_settings ] ) );
				$this->add_render_attribute( 'button', 'href', $current_link );
			}
