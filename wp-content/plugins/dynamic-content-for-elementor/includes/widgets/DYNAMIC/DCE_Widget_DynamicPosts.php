...
class DCE_Widget_DynamicPosts extends DCE_Widget_Prototype {
	...
	protected function _register_controls() {
		...
		/*$this->add_control(
			...
			'options' => Helper::get_acf_field_relational_post(), //get_acf_fields('relationship'),
			'default' => '0',
			'condition' => [
		...
		$this->add_control(
				'overflow_textzone_2',
				[
					'label' => __('Overflow Hidden', 'elementor'),
					'type' => Controls_Manager::SWITCHER,
					'return_value' => 'hidden',
					'selectors' => [
		...
		if ( $settings['filters_enable'] && $settings['posts_style'] == 'grid' ) {
			...
			if ( ! empty($term_list_filters) ) {
				...
				if ( $settings['all_filter'] ) {
					...
					echo '<' . $tag_filter . ' class="filters-item filter-active"><a href="#" data-filter="*">' . __($alltext, 'dynamic-content-for-elementor') . '</a></' . $tag_filter . '>' . $divisore_f;
				} else {
						...
						<div class="dce-wrapper <?php echo $animation_class; ?>">
							...
							if ( $settings['templatemode_enable'] == '' && $settings['native_templatemode_enable'] == '' ) {
								...
								if ( has_post_thumbnail() && $settings['show_image'] != 0 && $settings['order_image'] == '' ) {
									...
									// dalla versione 1.8.7 implemento gli effetti di animazione al rollhover del blocchi
								if ( $hoverEffects_enable ) {
					...
					<nav class="infiniteScroll">
						<div class="page-load-status<?php echo $preview_mode; ?>">
							...
							<div class="pagination" role="navigation">
								<a class="pagination__next" href="<?php echo \DynamicContentForElementor\Helper::get_next_pagination(); ?>"><?php //echo __('Next', 'dynamic-content-for-elementor'); ?></a>
							</div>
						</div>
	...
	private function generate_acfitems( $settings, $id_page = null ) {
		...
		$acf_name = false;
		/*if (isset($settings_acf_items['acf_items'][0]['__dynamic__']['link'])) {
			$link_dynamic = $settings_acf_items['acf_items'][0]['__dynamic__']['link'];
			$tag_data = \Elementor\Plugin::$instance->dynamic_tags->tag_text_to_tag_data($link_dynamic);
			if ($tag_data['name'] == 'acf-url') {
				list($acf_key, $acf_name) = explode(':', $tag_data['settings']['key']);
			}
		}*/

		/* $tag_value = \Elementor\Plugin::$instance->dynamic_tags->parse_tag_text($link_dynamic, array(), function() use ($link) {
		  $link =
		  }); */
	...
	public function generate_type( $settings, $post_id ) {
		...
		if ( $settings['type_label'] == 'singular' ) { //!empty($type->labels) && !empty($type->labels->singular_name)) {
			echo $type->labels->singular_name;
		} else {
			echo $type->label; //->plural_name;
		}
		if ( $settings['type_link'] ) {
			echo '</a>';
		}
