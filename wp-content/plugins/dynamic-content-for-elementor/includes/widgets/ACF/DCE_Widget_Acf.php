	...
	protected function render() {
		...
		if ( $typeField == 'text' ||
			...
			if ( $typeField == 'list' ) {
					...
					case 'checkbox':
						if ( ! empty($acf_field_value) ) {
							foreach ( $acf_field_value as $akey => $field ) {
								if ( $akey > 0 ) {
									$acfResult .= '<span class="dce-acf-list-separator">' . $settings['list_separator'] . '</span>';
								}
								if ( is_array($field) ) {
									if ( $settings['list_array_return'] == '' ) {
				...
				if ( $typeField == 'select' ) {
					$acfResult = __($acf_field_value, 'dynamic-content-for-elementor' . '_texts');
		...
		if ( $acfResult != '' || $typeField == 'empty' ) {
			...
			if ( $settings['acf_currency_mode'] != '' && $typeField != 'empty' && $settoDecimal == '' ) {
				?>
				<script>var acfValNum = new Number(<?php echo $acfResult; ?>).toLocaleString("<?php echo $currencytypeField; ?>");</script>
				<?php $acfResult = '<div class="edc-acf" id="edc-acf-cur"><script>document.getElementById("edc-acf-cur").innerHTML = acfValNum;</script></div>';
			}
			if ( $settings['acf_currency_mode'] == '' && $typeField != 'empty' && $settoDecimal != '' ) {
				?>
