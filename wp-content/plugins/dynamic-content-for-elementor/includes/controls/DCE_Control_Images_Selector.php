...
class DCE_Control_Images_Selector extends Base_Data_Control {
	...
	public function enqueue() {
		...
		wp_register_script( 'images-selector-control', plugins_url('/assets/js/images_selector-control.js', DCE__FILE__), [ 'jquery' ], DCE_VERSION );
			...
			<div class="elementor-control-input-wrapper">
				<div class="elementor-imageselector elementor-imageselector-type-{{{ data.type_selector }}}">
					...
					<div class="elementor-imageselector-item elementor-imageselector-column-{{{ columns_grid }}}" data-column-grid="{{{ columns_grid }}}">
						<input id="<?php echo $control_uid; ?>" type="radio" name="elementor-imageselector-{{ data.name }}-{{ data._cid }}" value="{{ valueItem }}">
						<label class="elementor-imageselector-label elementor-control-unit-1 tooltip-target" for="<?php echo $control_uid; ?>" data-tooltip="{{ options.title }}" title="{{ options.title }}">
