...
class DCE_Group_Control_Filters_CSS extends Group_Control_Base {
	...
	protected function init_fields() {
		...
		$controls['blur'] = [
			...
			'selectors' => [
				'{{SELECTOR}}' => 'filter: brightness( {{brightness.SIZE}}% ) contrast( {{contrast.SIZE}}% ) sepia( {{sepia.SIZE}} ) blur( {{blur.SIZE}}px ) invert( {{invert.SIZE}}%) hue-rotate( {{huerotate.SIZE}}deg) saturate( {{saturate.SIZE}}% )',
			],
			'condition' => [
				'filter_type' => 'custom',
			],
