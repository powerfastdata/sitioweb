...
function _dce_extension_form_step( $field ) {
	switch ( $field ) {
		case 'docs':
			return 'https://www.dynamic.ooo/widget/enhanced-multi-step-elementor-pro-form/';
		case 'description':
			return __('Add features to Elementor Pro Multi-Step: label as a legend, show all steps, scroll to top on step change and step summary', 'dynamic-content-for-elementor');
	}
}
...
if ( ! Helper::is_elementorpro_active() ) {
	...
	class DCE_Extension_Form_Step extends DCE_Extension_Prototype {
		...
		public $name = 'Enhanced Multi-Step for Elementor Pro Form';
	...
	class DCE_Extension_Form_Step extends DCE_Extension_Prototype {
		...
		public function add_control_section_to_steps( $element, $args ) {
			...
			$element->add_control(
					'dce_step_summary',
					[
						'label' => '<span class="color-dce icon icon-dyn-logo-dce"></span> ' . __('Enable Step Summary', 'dynamic-content-for-elementor'),
						'type' => \Elementor\Controls_Manager::SWITCHER,
