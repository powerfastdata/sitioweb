...
if ( ! Helper::is_plugin_active( 'elementor-pro' ) ) {
	...
	class DCE_Extension_Form_PayPal extends \ElementorPro\Modules\Forms\Fields\Field_Base {
		...
		public function render( $item, $item_index, $form ) {
			...
			$form->add_render_attribute( 'paypal-buttons' . $item_index,
				'data-currency', esc_attr( $item['dce_form_paypal_currency'] ) );
			$form->add_render_attribute( 'paypal-buttons' . $item_index,
			...
			if ( 'yes' === $settings['dce_paypal_center'] ) {
				$form->add_render_attribute( 'paypal-buttons' . $item_index, 'style', 'margin: auto;');
			}
			echo '<div style="width: 100%; padding-top: 2px;">';
		...
		public function update_controls( $widget ) {
			...
			if ( \Elementor\Plugin::$instance->editor->is_edit_mode()
				...
				$field_controls = [
					'dce_form_paypal_currency' => [
						'name' => 'dce_form_paypal_currency',
		...
		public static function paypal_client() {
			if ( get_option('dce_paypal_api_mode', 'sandbox') === 'sandbox' ) {
				...
				$env = new LiveEnvironment($client_id, $client_secret);
			}
		...
		private static function get_field_settings( $id, Classes\Form_Record $record ) {
			...
			$field_settings = array_filter($field_settings, function( $field ) use ( $raw_id ) {
				return $field['_id'] === $raw_id;
			});
		...
		private static function get_parsed_item_value( $field_settings ) {
			...
			if ( $document ) {
				$form = \ElementorPro\Modules\Forms\Module::find_element_recursive( $document->get_elements_data(), $_POST['form_id'] );
			}
		...
		public function validation( $field, Classes\Form_Record $record, Classes\Ajax_Handler $ajax_handler ) {
			...
			try {
				...
				$settings_value = Helper::get_dynamic_value( $field_settings );
				$settings_currency = $field_settings['dce_form_paypal_currency'];
