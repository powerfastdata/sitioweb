...
if ( ! Helper::is_elementorpro_active() || ! class_exists('ElementorPro\Modules\Forms\Classes\Action_Base') ) {
	...
	class DCE_Extension_Form_Email extends DCE_Extension_Prototype {
		...
		public $name = 'Dynamic Email for Elementor Pro Form';
	...
	class DCE_Extension_Form_Email extends \ElementorPro\Modules\Forms\Classes\Action_Base {
		...
		public function register_settings_section( $widget ) {
			...
			$repeater_fields->add_control(
					'dce_form_email_condition_field', [
						...
						'description' => __('Type here the ID of the form field to check, or leave empty to always send this email', 'dynamic-content-for-elementor'),
					]
			);
			$repeater_fields->add_control(
				...
				if ( $amail['dce_form_email_enable'] ) {
					...
					if ( $condition_satisfy ) {
						...
						if ( $use_template ) {
							...
							$attachments = $this->get_email_attachments($dce_form_email_content, $fields, $amail, $settings);
		...
		public function get_email_attachments( $dce_form_email_content, $fields, $amail, $settings ) {
			$attachments = array();
		...
		public static function add_dce_email_template_type() {
			...
			$dce_email = 'Elementor\Modules\Library\Documents\DCE_Email';
			\Elementor\Plugin::instance()->documents->register_document_type( $dce_email::get_name_static(), \Elementor\Modules\Library\Documents\DCE_Email::get_class_full_name() );
		}
	}
}
