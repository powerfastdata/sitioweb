...
function _dce_extension_form_save( $field ) {
	switch ( $field ) {
		...
		case 'description':
			return __('Save the form data submitted by your client as posts, users or terms.', 'dynamic-content-for-elementor');
	}
}
...
if ( ! Helper::is_elementorpro_active() || ! class_exists('ElementorPro\Modules\Forms\Classes\Action_Base') ) {
	...
	class DCE_Extension_Form_Save extends DCE_Extension_Prototype {
		...
		public $name = 'Save for Elementor Pro Form';
	...
	class DCE_Extension_Form_Save extends \ElementorPro\Modules\Forms\Classes\Action_Base {
		...
		public function register_settings_section( $widget ) {
			...
			$widget->add_control(
					'dce_form_save_file', [
						...
						'type' => Controls_Manager::SWITCHER,
						'description' => __('Create a Media and save his ID instead of the URL', 'dynamic-content-for-elementor'),
					]
			);
			$widget->add_control(
					'dce_form_save_array', [
			...
			$widget->add_control(
					'dce_form_save_type_obj_id', [
						...
						'label_block' => true,
						'description' => __('ID of the Object to update or leave empty for Current.', 'dynamic-content-for-elementor'),
						'condition' => [
			...
			$widget->add_control(
					'dce_form_save_type_post_post', [
						...
						'type' => 'ooo_query',
						'placeholder' => __('Select a Post or leave empty for the Current one', 'dynamic-content-for-elementor'),
						'label_block' => true,
						'query_type' => 'posts',
						'description' => __('Search Post by Title or leave empty for Current Post.', 'dynamic-content-for-elementor'),
						'condition' => [
							'dce_form_save_type' => 'post',
							'dce_form_save_override!' => '',
							'dce_form_save_type_obj_id' => '',
			...
			$widget->add_control(
					'dce_form_save_type_user_user', [
						...
						'label_block' => true,
						'query_type' => 'users',
						'description' => __('Search User or leave empty for Current User.', 'dynamic-content-for-elementor'),
						'condition' => [
							'dce_form_save_type' => 'user',
							'dce_form_save_override!' => '',
							'dce_form_save_type_obj_id' => '',
			...
			$widget->add_control(
					'dce_form_save_type_term_term', [
						...
						'label_block' => true,
						'query_type' => 'terms',
						'description' => __('Search Term or leave empty for Current Term.', 'dynamic-content-for-elementor'),
						'condition' => [
			...
			$widget->add_control(
					'dce_form_save_type_post_title', [
						...
						'description' => __('Can use static text, field shortcode, tokens or mixed', 'dynamic-content-for-elementor') . '<br>' . __('or leave it empty for rand value', 'dynamic-content-for-elementor'),
						'condition' => [
							'dce_form_save_type' => 'post',
						],
						'label_block' => 'true',
			...
			$widget->add_control(
					'dce_form_save_type_user_error_stop', [
						...
						'type' => Controls_Manager::SWITCHER,
						'description' => __('Stop next actions execution on Error, username or email are not valid', 'dynamic-content-for-elementor'),
						'condition' => [
							'dce_form_save_type' => 'user',
							'dce_form_save_override' => '',
						],
		...
		function dce_elementor_form_save( $record, $settings = null, $ajax_handler = null ) {
			...
			if ( $obj_id ) {
				...
				if ( ! empty($fields) && is_array($fields) ) {
					if ( ! empty($settings['dce_form_save_metas']) && is_array($settings['dce_form_save_metas']) ) {
		...
		function get_obj_id( $obj_id, $type, $ajax_handler ) {
			$obj_id = Helper::get_dynamic_value($obj_id);
			if ( is_string($obj_id) && is_numeric($obj_id) ) {
				$obj_id = intval($obj_id);
			}
			if ( ! $obj_id ) {
				$ajax_handler->add_error_message(__($type . ' ID not valid', 'dynamic-content-for-elementor'));
				return false;
			}
			switch ( $type ) {
				case 'post':
			...
			if ( ! $obj_check ) {
				$ajax_handler->add_error_message(__($type . ' not existent', 'dynamic-content-for-elementor'));
				return false;
			}
			return $obj_id;
		}
