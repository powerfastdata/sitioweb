var isAdminBar=!1,isEditMode=!1;(function($){var DyncontEl_GoogleMapsHandler=function($scope,$){var map;var bounds;var elementSettingsMap=get_Dyncontel_ElementSettings($scope);var id_scope=$scope.attr('data-id');var map=$scope.find('#el-wgt-map-'+id_scope)[0];var indirizzo=jQuery(map).data('address');var lati=jQuery(map).data('lat')||0;var long=jQuery(map).data('lng')||0;var infoWindow=jQuery(map).data('infowindow')||'Empty Info Window';var lastWindow=null;var imageMarker=jQuery(map).data('imgmarker')||'';var w=elementSettingsMap.marker_width;var h=elementSettingsMap.marker_height;if(w&&h&&imageMarker){imageMarker={url:jQuery(map).data('imgmarker')||'',scaledSize:new google.maps.Size(w,h)}}
...
if(elementSettingsMap.style_select==='custom'){mapParams.styles=eval(elementSettingsMap.style_map);fireMap(map,mapParams)}else if(elementSettingsMap.style_select==='prestyle'){var fileStyle=elementSettingsMap.snazzy_select;$.getJSON(fileStyle+".json",function(json){mapParams.styles=json;fireMap(map,mapParams)})}else{fireMap(map,mapParams)}
function fireMap(elements_map,mapParams_map){map=new google.maps.Map(elements_map,mapParams_map);var labels='ABCDEFGHIJKLMNOPQRSTUVWXYZ';var markers=[];var mapDataType=elementSettingsMap.map_data_type;if(elementSettingsMap.acf_mapfield){mapDataType='latlng'}
...
function changeMapLocation(locations){if(locations&&locations.length>=1){console.log(locations[0]);var image={url:locations[0].marker,};var shape={coords:[1,1,1,20,18,20,18,1],type:'poly'};var objMarker={map:map,position:locations[0].location,};if(locations[0].marker!=""){objMarker.icon=image}
...
elementorFrontend.hooks.addAction('frontend/element_ready/dyncontel-acf-google-maps.default',DyncontEl_GoogleMapsHandler)})})(jQuery)