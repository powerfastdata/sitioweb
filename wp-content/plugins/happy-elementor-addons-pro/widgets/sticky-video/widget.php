...
class Sticky_Video extends Base {
        ...
        $this->add_control(
            ...
            [
                ...
                'options' => [
                    ...
                    //'wistia'  => __( 'Wistia', 'happy-addons-pro' ),
    ...
    protected function register_style_controls() {
        ...
        $this->start_controls_section(
            'sticky_video_player_interface_section',
            [
                'label' => __('Interface', 'happy-addons-pro'),
                'tab' => Controls_Manager::TAB_STYLE,
            ]
        ); */
        ...
        $this->add_control(
            ...
            [
                ...
                'selectors' => [
                    ...
                    // '{{WRAPPER}} .plyr__control.plyr__tab-focus:focus' => 'box-shadow: 0 0 0 5px {{VALUE}}!important',

                    // '{{WRAPPER}} .plyr__control--overlaid' => 'background: {{VALUE}}!important',
                    // '{{WRAPPER}} .plyr__control--overlaid:focus' => 'background: {{VALUE}}!important',

                    // '{{WRAPPER}} .plyr--video .plyr__control.plyr__tab-focus' => 'background: {{VALUE}}!important',
                    // '{{WRAPPER}} .plyr--video .plyr__control.plyr__tab-focus:focus' => 'background: {{VALUE}}!important',

                    // '{{WRAPPER}} .plyr__control--overlaid' => 'background: {{VALUE}}!important',
                    // '{{WRAPPER}} .plyr__control--overlaid:focus' => 'background: {{VALUE}}!important',

                    // '{{WRAPPER}} .plyr--video .plyr__control:hover' => 'background: {{VALUE}}!important',
                    // '{{WRAPPER}} .plyr--video .plyr__control:focus' => 'background: {{VALUE}}!important',
                ],
            ]
        );
        ...
        $this->start_controls_section(
            'sticky_video_player_bar_section',
            [
    ...
    protected function render() {
        ...
        //var_dump($this->get_url_id($settings));
        ...
        if ( 'yes' === $settings['overlay_options'] ) {
            ...
            //     $image = Group_Control_Image_Size::get_attachment_image_src( $settings['overlay_image']['id'], 'overlay_image_size', $settings );
            // }
            if( $image ) {
                $this->add_render_attribute (
                    'overlay',
        ...
        //echo '<pre>';
        //var_dump( $settings['overlay_image'] );
        // var_dump($settings['play_icon']);
        //echo '</pre>';
    ...
    protected function video_player($settings) {
        ...
        if( 'youtube' === $settings['video_type'] && 'yes' !== $settings['loop'] && $settings['start'] && $settings['end'] ) {
            //$config .= '"youtube": {"start": '.$settings['start'].',"end": '.$settings['end'].'}';
